...
FMP changed their API from `v3` to `stable`. Our code still calls the old `v3` URLs → every call fails.
...
| **Quote** | PE ratio, EPS, earnings date for LEAPS | 🟡 LEAPS lack fundamentals |
...
On weekends → model is off → all Greeks = 0.

```
Monday-Friday (market hours):  delta=0.45, gamma=0.03, theta=-0.12  ✅
Saturday-Sunday:               delta=0,    gamma=0,    theta=0      ❌
```
...
- ✅ Stock price (from ORATS)
- ✅ Strike price (from the option)
- ✅ Time to expiry (calculated)
- ✅ Interest rate (use 10-year Treasury ~4.5%)
...
```mermaid
flowchart TD
    A[Need Greeks for AI Analysis] --> B{ORATS Delta ≠ 0?}
    B -->|Yes| C["Use ORATS Greeks (live) ✅"]
    B -->|No, it's 0| D{ORATS IV smvVol available?}
    D -->|Yes| E["Calculate via Black-Scholes ✅<br/>Label as (estimated)"]
    D -->|No| F{Tradier API available?}
    F -->|Yes| G["Use Tradier Greeks ✅<br/>Label as (Tradier)"]
    F -->|No| H["Tell AI: Greeks unavailable ✅<br/>AI adjusts expectations"]
```
...
| **AVGO** | Pullback bullish | Weak (z=-0.6) | Not recent | **32** | ~35 ✅ |
| **JPM Call** | Pullback bullish | Normal | EPS beat | **62** | ~65 ✅ |
...
```
tech_score = 53       → +1.8 points
sentiment = 100       → +20 points  
volume z-score = 2.1  → +10 points (surging)
ma_signal = bullish   → +10 points

Base Score = 50 + 1.8 + 20 + 10 + 10 = 91.8 → clamped to 90
AI range: [70, 100]   ← AI can lower for OTM/illiquidity but floor is 70
```
...
flowchart TD
    ...
    E -->|Yes| F["Accept Score ✅"]
    E -->|No, too far off| G["Log warning ⚠️<br/>Use base score instead"]
