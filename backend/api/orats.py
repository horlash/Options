    ...
    def _standardize_response(self, orats_data):
        ...
        for item in raw_list:
            ...
            call_obj = {
                ...
                "bid": item.get("callBid", 0),
                "ask": item.get("callAsk", 0),
                "last": item.get("callPrice", 0), 
                "mark": item.get("callValue", 0) or ((item.get("callBid",0) + item.get("callAsk",0))/2), 
                "totalVolume": item.get("callVolume", 0),
                "openInterest": item.get("callOpenInterest", 0),
                "volatility": item.get("callIvInfinity", 0) * 100 if item.get("callIvInfinity") else 0, # Check key name!
                "delta": item.get("callDelta", 0),
                "gamma": item.get("callGamma", 0),
                "theta": item.get("callTheta", 0),
                "vega": item.get("callVega", 0),
                "rho": item.get("callRho", 0),
            ...
            put_obj = {
                ...
                "bid": item.get("putBid", 0),
                "ask": item.get("putAsk", 0),
                "last": item.get("putPrice", 0),
                "mark": item.get("putValue", 0) or ((item.get("putBid",0) + item.get("putAsk",0))/2),
                "totalVolume": item.get("putVolume", 0),
                "openInterest": item.get("putOpenInterest", 0),
                "volatility": item.get("putIvInfinity", 0) * 100 if item.get("putIvInfinity") else 0,
                "delta": item.get("putDelta", 0),
                "gamma": item.get("putGamma", 0),
                "theta": item.get("putTheta", 0),
                "vega": item.get("putVega", 0),
                "rho": item.get("putRho", 0),
                "strikePrice": float(strike),
                "expirationDate": expiry,
                "daysToExpiration": days_to_expiry
            }
