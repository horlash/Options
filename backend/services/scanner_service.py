...
from backend.api.td_ameritrade import TDAmeritradeAPI
from backend.api.news_apis import NewsAPIs
...
class ScannerService:
    def __init__(self):
        self.td_api = TDAmeritradeAPI()
        self.news_api = NewsAPIs()
    ...
    def scan_ticker(self, ticker):
        ...
        try:
            ...
            quote = self.td_api.get_quote(ticker)
            ...
            price_history = self.td_api.get_price_history(ticker)
            ...
            options_data = self.td_api.get_options_chain(ticker)
            
            if not options_data:
                print(f"❌ Failed to get options chain")
                return None
